...
\red{i would maybe rephrase this slightly to sound less...conniving :)    maybe more like 'in trying to understand whether or not massive galaxies can form blobs as a natrual part of their evolution, we find many successes and failures.   [and then summarize them].   i definitely get the sentiment that you're going for, but i guess i'm not sure hwo we would actually use our models to rpove that we can't form blobs (so, if you want to go down that route, maybe expand on that a bit more quantiatiavely). } In the work presented here, we deliberately argued that our simulations reproduce Ly$\alpha$ blobs, but if we were so motivated we could use the results of our radiative transfer calculations to construct a solid argument that these objects are not definitely the same class of object as observed blobs.
...
Our primary improvements over other works are that we account for emission due to recombinations and collisional excitations, we compute rates for both those processes based on first principles, and we do Ly$\alpha$ Monte-Carlo radiative transfer on  high(er)-resolution simulations. \red{what about the inclusion of AGN?  what about the ionization RT? maybe build this up a bit and discuss how this is the first work to include {\it all} of XYZ physical processes that we think might be important.}
...
If there is one lesson from this work it is that radiative transfer rapidly becomes complex, so we are very wary of such an approximation. \red{we should talk about the temperature dependence of Lya and how this can all impact this.}
...
\red{i feel like this section could either be beefed up, or maybe it should be axed.  it kind of doesn't add a lot as its currently written.}
...
The one downside to the HDF5 format is that there is a single implementation, a C library which is produced by the HDF group, which is not without flaws. \red{There is a telling anecdote in the scientific progress of this thesis.}

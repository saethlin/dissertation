...
To be clear, the work we have done here in post-processing is not a panacea; this must be done on-the-fly because all computations done in post-processing will not be self-consistent.
...
Additionally, we do not have any guarantee that these outputs are actually converged.
...
Observe that in principle an octree domain could be any rectangular prism, and in that case we would position for each octant, and there would be three side lengths.

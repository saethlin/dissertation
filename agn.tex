...
That is, though the black hole particles in this simulation will accrete mass, there is no prescription for the flow of thermal and kinetic energy into the surrounding gas from the accretion, \red{well we care about the ionization right?  so i'm not 100\% sure this remainder of this sentence is true?} which is what makes black holes astrophysically interesting in this context.
...
In this model, the AGN SED is modeled by employing the \citet*{Hopkins2007} templates for unreddened quasars, with the luminosity being tied to the accretion rate via $L = \eta \dot{M_{\rm BH}} c^2$, \red{DEFINE MDOT please} where $\eta$ is an efficiency parameter with a fiducial value of $\eta = 0.1$. 
...
\section{Large-Scale Morphological Changes}\red{i love this figure!  would it be possible to also include a figure showing a map of the ionization state for a model wtih stars only and stars and AGN?  if its difficult we shouldn't do it - just wondering if its easy.  also for 4-1 through 4-3 lets put column titles saying "no-AGN", "AGN" etc. please}We include Figure \ref{fig:agn_rogues1}, \ref{fig:agn_rogues2}, \ref{fig:agn_rogues4}, and \ref{fig:agn_rogues8} to orient the reader as to the visual effects of AGN on our model.
...
\red{i'm not sure what you mean by pale and darker?   like in 3-12 or in the morphology plots?}As in Figure~\ref{fig:recombination_collision}, the distance between the pale and darker lines provides a visualization of the escape fraction along a median line of sight.
...
A precise explanation of this is \red{outside the scope of our current study}, but for now we can offer a few explanations.
...
\red{i love this section} There are two features in these plots that deserve specifc explanation: At some redshifts the escape fraction range of the AGN model extends below the no-AGN model, and at some redshifts the median escape fraction with AGN passes below that of the no-AGN model.
...
This gives us an order of magnitude variation in either direction from our best-guess model and so should encompass any relevant trends in this parameter; we plot the results of this experiment in Figure~\ref{fig:eddington_grid}. \red{i dont think this is the right figure?  or the caption of that figure is wrong anyways.   also need to spend some time in the text explaning what the figure actually is since its not explained here.}

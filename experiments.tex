...
As as apparent from the \red{aforementioned text in} \sout{structure of} this dissertation, the structure of the project has shifted since then, most notably to consider the underlying physical origin of Ly$\alpha$ as opposed to the astrophysical origins \red{not really sure what this last sentence means}.

\red{I get what everything after this means, but the transition from the first few sentences to here is awkward...i'm not really sure waht the sentences before this means, and how this gets to us wanting to understand the impact of the UV background.  If you can clarify that would be awesome.}But in the case of the UV background we have an explicit simulation of this, in the ionizing radiative transfer code that we use to compute the ionization state we can adjust the intensity of the UV background that is simulated.
...
We increase the intensity of the UV background from a best-guess value \red{what is the "best-guess" value?} in \citet{Faucher-Giguere2009} up to a non-physical increase of 5 orders of magnitude (Figures~\ref{fig:big_uv_000} and \ref{fig:big_uv_005}), and find that this provides only a small enhancement to the luminosity of the blob and if anything makes it less like a blob by decreasing its spatial extent.
...
\begin{figure*}
    ...
    \caption{
        A surface brightness image of a sample halo, with our best-guess \red{not sure what "best-guess" is - please clarify} UV background from \citet{Faucher-Giguere2009}.
    }
...
Ideally, we'd like to know if our simulation \red{did you just say we won't call the RT simulations? am i misunderstanding your usage of the word in this sentence?}is converged.
...
The reason this simplistic metric is wrong is beyond the scope of this work \red{can we give a 1 sentence explanation as to whats going on, or at least why its outsdie the scope of this work?  right now the reader has no idea why its outside the scope}.
